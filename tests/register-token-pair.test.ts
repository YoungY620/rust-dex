...
describe("rust-dex: register_token_pair", () => {
  ...
  it("Should register token pair", async () => {
    ...
    expect(tokenPairAccount.baseToken.toString()).to.equal(mint1.toString());
    expect(tokenPairAccount.quoteToken.toString()).to.equal(mint2.toString());
    // expect(tokenPairAccount.bump.toNumber()).to.be.a('number');

    console.log("Token Pair Account:", {
      baseToken: tokenPairAccount.baseToken.toString(),
      quoteToken: tokenPairAccount.quoteToken.toString(),
    ...
    expect(oppositePairAccount.baseToken.toString()).to.equal(mint2.toString());
    expect(oppositePairAccount.quoteToken.toString()).to.equal(mint1.toString());
    expect(oppositePairAccount.bump).to.be.a('number');

    console.log("Opposite Token Pair Account:", {
      baseToken: oppositePairAccount.baseToken.toString(),
      quoteToken: oppositePairAccount.quoteToken.toString(),
